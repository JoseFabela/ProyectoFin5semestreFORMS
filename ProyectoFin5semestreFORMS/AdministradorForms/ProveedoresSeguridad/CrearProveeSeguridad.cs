...
{
    public partial class CrearProveeSeguridad : Form
    {
        static string connectionString = "Server=localhost;Database=ProyectoF5Sem;Integrated Security=True;";

        public CrearProveeSeguridad()
        {
            InitializeComponent();
        }

        private void CrearProveeSeguridad_Load(object sender, EventArgs e)
        {
        ...
        private bool Crear(string nombre, string servicio)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string query = @"INSERT INTO proveedor_seguridad (nombre, servicio, estado)
                                     VALUES (@nombre, @servicio, 'Activo')";
